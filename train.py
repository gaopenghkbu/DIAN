...
#criterion_con = Frequency_ContrastiveLoss(batch_size=args.batch_size)
# criterion_cpm= CPMOOLoss(margin=0.2)
# criterion_cpm = Frequency_ContrastiveLoss(batch_size=args.batch_size)
criterion_cpm = PLoss(margin=0.2)
criterion_ort = Frequency_ContrastiveLoss(batch_size=args.batch_size)

criterion_id.to(device)
criterion_tri.to(device)
criterion_ort.to(device)
criterion_cpm.to(device)

if args.optim == 'sgd':
    ignored_params =   list(map(id, net.bottleneck.parameters())) \
                     + list(map(id, net.classifier.parameters())) \
                     # + list(map(id, net.MFA1.parameters())) \
                     # + list(map(id, net.MFA2.parameters())) \
                     # + list(map(id, net.MFA3.parameters())) \
    ...
    #     {'params': net.MFA1.parameters(), 'lr': args.lr},
    #     {'params': net.MFA2.parameters(), 'lr': args.lr},
    #     {'params': net.MFA3.parameters(), 'lr': args.lr},
    #     ],
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        #print('ft0', ft0.shape)
        #loss_con = (criterion_con(torch.cat((ft0, ft1), 0), labs) + criterion_con(torch.cat((ft0, ft3), 0), labs) + criterion_con(torch.cat((ft0, ft5), 0), labs) + criterion_con(torch.cat((ft1, ft3), 0), labs) + criterion_con(torch.cat((ft1, ft5), 0), labs) + criterion_con(torch.cat((ft3, ft5), 0), labs)) * args.lambda_1
        #loss_cpm = (criterion_cpm(torch.cat((ft1, ft3), 0), labs) + criterion_cpm(torch.cat((ft3, ft5), 0), labs))* args.lambda_1
        loss_cpm = (criterion_cpm(torch.cat((ft1, ft3), 0), labs) + criterion_cpm(torch.cat((ft3, ft5), 0), labs) + criterion_cpm(torch.cat((ft0, ft3), 0), labs))* args.lambda_1
        loss_ort = (criterion_ort(torch.cat((ft1, ft3), 0), labs) + criterion_ort(torch.cat((ft3, ft5), 0), labs) + criterion_ort(torch.cat((ft0, ft3), 0), labs))* args.lambda_1
        #print('lll')
        #loss_cpm = (criterion_cpm(torch.cat((ft0, ft3), 0), labs)) *  args.lambda_1
        # loss_ort = loss_ort *  args.lambda_2
        # loss = loss_id + loss_tri + loss_cpm + loss_ort
        # loss = loss_id + loss_tri + loss_cpm
        loss = loss_id + loss_tri + loss_cpm + loss_ort
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    #writer.add_scalar('con_loss', con_loss.avg, epoch)
...
def test(epoch):
    ...
    if dataset == 'regdb':
        ...
        cmc2, mAP2, mINP2 = eval_llcm(-distmat2, query_label, gall_label, query_cam, gall_cam)
        cmc4, mAP4, mINP4 = eval_llcm(-distmat4, query_label, gall_label, query_cam, gall_cam)
        cmc6, mAP6, mINP6 = eval_llcm(-distmat6, query_label, gall_label, query_cam, gall_cam)
